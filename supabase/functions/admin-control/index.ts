...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    if(action==="delete_customer"){
      ...
      await deleteAuthUser(admin,targetId);
      await admin.from("admin_audit_logs").insert({admin_id:user.id,action_type:"customer.account_deleted",target_id:targetId,details_json:{auth_identity_deleted:true,personal_data_anonymized:true,order_analytics_preserved:true}}).throwOnError();
      return respond({ok:true});
    }
    if(action==="system_reset"){
      ...
      if(reset==="customers"||reset==="factory"){
        ...
        while(true){
          ...
          for(const customer of batch){await deleteAuthUser(admin,customer.id);deletedAuthIds.push(customer.id);}
