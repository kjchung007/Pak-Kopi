...
const opened=await fetch(base+'/preview/draft-access?token='+result.token,{redirect:'manual'});assert.equal(opened.status,307);assert.equal(opened.headers.get('location'),'/preview/');assert(opened.headers.get('set-cookie').includes('Path=/preview'));
