...
for(const s of catalog.stores)if(Number.isFinite(s.latitude)&&Number.isFinite(s.longitude))sql+=`update public.stores set latitude=${s.latitude},longitude=${s.longitude} where id=${s.id} and latitude is null and longitude is null;\n`;
