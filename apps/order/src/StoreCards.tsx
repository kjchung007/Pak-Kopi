...
export function StoreCards({stores,selected,loading,error,choose}:{stores:Store[];selected:Store|null;loading:boolean;error:string;choose:(s:Store)=>void}){
 ...
 const hours=(s:Store)=>{if(s.opening_time===s.closing_time)return 'All-day pickup';const format=(v:string)=>{const [h,m]=v.split(':').map(Number);return `${h%12||12}:${String(m).padStart(2,'0')} ${h>=12?'pm':'am'}`;};return `${format(s.opening_time)} – ${format(s.closing_time)}`;};
 return <div className="pickup-discovery">
  ...
  <span className={position||region!=='nearby'?'pickup-sr-only':'pickup-location-status'} role="status">{status}</span>
