...
import "./classic.css";
import { House, Coffee, Receipt, Gift, UserCircle, Plus, Leaf, Cookie, Sparkle } from "@phosphor-icons/react";
...
type SignupResult = "signed-in" | "check-email" | false;
type CartLine = {
  key: string;
  itemId: number;
  quantity: number;
...
type CustomerOrder = {
  ...
  payment_method: "fpx" | "touch_n_go" | "card" | null;
...
export default function App() {
  ...
  const confirmStore = (store: Store) => {
    if (selectedStore && store.id !== selectedStore.id && cartCount > 0) {
      if (!window.confirm("Changing branches clears your current cart. Continue?")) return;
      setCart({}); cartRef.current = {}; setPaymentDraft(null);
    }
    selectStore(store);
  ...
  const submitSignUp = async (displayName: string): Promise<SignupResult> => {
    setAuthError("");
    ...
    if (error) {
      ...
      setAuthError(duplicate ? "An account with this email already exists. Sign in or reset your password." : error.message);
      return false;
    }
  ...
  const signInWithProvider = async (provider: "google" | "apple") => {
    setAuthError("");
    if (import.meta.env.VITE_LOCAL_DEMO === "true") { setAuthError("Use an email and password for this local demo. Social sign-in is not connected."); return; }
    if (provider === "apple") {
  ...
  const reorder = (order: CustomerOrder) => {
    ...
    if (order.stores && selectedStore?.id !== order.stores.id && cartCount > 0 && !window.confirm("Reordering from another branch clears your cart. Continue?")) return;
    setCart(selectedStore?.id === order.stores?.id ? {...cartRef.current,...next} : next);
    cartRef.current = selectedStore?.id === order.stores?.id ? {...cartRef.current,...next} : next;
  ...
  const placeOrder = async (
    paymentMethod: "fpx" | "touch_n_go" | "card",
    totalAmount: number,
    ...
    const voucherKey = voucher?.userVoucherId
      ...
      : voucher?.secretCode
        ...
        : "none";
    const fingerprint = JSON.stringify([selectedStore.id, cartItems.map(({item,line})=>[item.id,line.quantity,line.size,line.temperature])]);
    let orderId =
      paymentDraft?.fingerprint === fingerprint &&
    ...
    if (hitpaySandbox) {
      await startSandboxPayment(orderId);
      return;
    }
    const {error} = await supabase.rpc('complete_demo_payment', {p_order_id:orderId});
    setOrderBusy(false);
    if(error){setAuthError(error.message);return;}
    setPlacedOrder(true);setPaymentDraft(null);setCart({});cartRef.current={};
    await supabase.rpc('replace_customer_cart',{p_store_id:selectedStore.id,p_items:[]});
    setCartOpen(false);setTab('orders');await loadOrders();

  };

  async function startSandboxPayment(orderId: number) {
    setOrderBusy(true);
    try {
      const {data,error} = await supabase.functions.invoke('create-hitpay-payment', {body:{order_id:orderId,origin:window.location.origin}});
      if(error) {
      ...
      <div className={`scroll-area${tab === "home" ? " scroll-home" : ""}`}>{brand.showDemoBanner !== false && <div className="demo-notice">{brand.notice} · {brand.menuShortNote}</div>}
        <header className={`topbar${tab === "home" ? " topbar-home" : ""}`}>
          ...
          >
            {settings.logo_url ? (
              ...
              <img className="brand-mark brand-image" src={brand.logo} alt="" />
            )}
            <span>
        ...
        {tab === "orders" && (
          <OrdersPage
            ...
            resumePayment={(id) => void startSandboxPayment(id)}
            paymentBusy={orderBusy}
          />
        )}
        {tab === "rewards" && (
          <RewardsPage
    ...
    <main className="home-page">
      ...
      {campaign ? (
        <section
          ...
          style={{
            backgroundImage: `linear-gradient(90deg,color-mix(in srgb, var(--brand-deep) 86%, transparent),color-mix(in srgb, var(--brand-deep) 18%, transparent)),url(${campaign.image})`,
          }}
        ...
        <section className="empty-state">
          <h2>No featured campaigns</h2><p>Published campaigns will appear here.</p>
        </section>
      )}
...
function CategorySymbol({name}:{name:string}) {const Symbol=name.toLowerCase().includes("tea")?Leaf:name.toLowerCase().includes("chocolate")?Cookie:name.toLowerCase().includes("special")?Sparkle:Coffee;return <Symbol className="category-symbol" size={20} aria-hidden />;}
function MenuPage({
  menu,
  categories,
  activeCategory,
      ...
      <main className="menu-layout">
        <aside className="category-rail" aria-label="Menu categories">
          {categories.map((category) => (
            ...
            >
              <CategorySymbol name={category.name} />
        ...
        <aside className="desktop-summary" aria-label="Current order summary">
          ...
          <span>Current order</span><h2>Your pickup</h2>
          {cartCount === 0 ? (
            <p>
              {storeIsOpen
        ...
        <section className="order-group active-group">
          ...
          {active.map((order) => (
            <div className="active-order-card" key={order.id}>
              ...
              {hitpaySandbox && order.payment_status === 'pending' && <button disabled={paymentBusy} onClick={() => resumePayment(order.id)}>Continue test payment</button>}
  ...
  return (
    <div className="order-detail-layer">
      ...
      >
        ...
        {order.payment_status === "pending" && <div className="payment-recovery pending" role="status"><strong>Verifying your payment</strong><span>Payment is awaiting confirmation. Preparation starts only after the payment is verified.</span></div>}
        ...
        <main>
          <section className="detail-card pickup-detail">
            ...
            <span>
              ...
              <strong>{order.stores?.name ?? brand.name}</strong>
...
function RewardsPage({
  ...
  const lastCelebratedId = useRef<number | null>(null),
    ...
    autoClaimingRef = useRef(false);

  // Automatically claim completed stamp card in the background if stamps reached threshold
  useEffect(() => {
    if (
      settings.stamp_enabled &&
      settings.stamp_threshold > 0 &&
      ...
      void (async () => {
        try {
          ...
          if (!error && data?.ok && data?.claimed > 0) {
            const first = data.vouchers?.[0];
            await refresh();
            if (first?.id) {
              try {
                const key = "coffee-demo-seen-stamp-vouchers";
                const seen: number[] = JSON.parse(
                  localStorage.getItem(key) || "[]",
                );
                if (!seen.includes(Number(first.id))) {
                  localStorage.setItem(
                    key,
                    JSON.stringify([...seen, Number(first.id)]),
                  );
                }
              } catch {}
              announceGrant({ id: Number(first.id), source: "stamp_reward" });
            }
          }
        } catch (err) {
          console.error("Auto-claim stamp error:", err);
        } finally {
  ...
  // Fallback: If a stamp_reward voucher exists in active vouchers but has not played celebration animation yet
  useEffect(() => {
    try {
      const key = "coffee-demo-seen-stamp-vouchers";
      ...
      if (uncelebrated && !pendingStampVoucherId) {
        localStorage.setItem(key, JSON.stringify([...seen, uncelebrated.id]));
        announceGrant({ id: uncelebrated.id, source: "stamp_reward" });
      }
    } catch {}
  }, [vouchers, pendingStampVoucherId, announceGrant]);
  const now = Date.now(),
  ...
  useEffect(() => {
    ...
    try {
      if (rewardGrant.source === "stamp_reward") {
        const key = "coffee-demo-seen-stamp-vouchers";
        const seen: number[] = JSON.parse(localStorage.getItem(key) || "[]");
        if (!seen.includes(rewardGrant.id)) {
          localStorage.setItem(key, JSON.stringify([...seen, rewardGrant.id]));
        }
      }
    } catch {}
    setView("mine");
  ...
  return (
    <main className={`rewards-page rewards-${view}`}>
      <header className="rewards-hero">
        <div>
          <span>{brand.shortName.toUpperCase()} MEMBERS</span>
        ...
        <>
          ...
          <form className="secret-code" onSubmit={claim}>
            <div>
              ...
              <strong>Unlock a {brand.shortName} treat</strong>
            </div>
            <label>
            ...
            <small>
              Demo codes to try: DEMOWELCOME · DEMOBUY2
            </small>
          </form>
  ...
  return (
    <main className="simple-page profile-page">
      ...
      <section className="profile-identity">
        ...
        <div>
          <strong>
            {draft.display_name || email.split("@")[0] || `${brand.shortName} member`}
          </strong>
          <small>{email}</small>
        </div>
      </section>
      ...
      <nav className="profile-links" aria-label="More information">
        <a href={brand.contact.email ? `mailto:${brand.contact.email}` : `${customerWebsiteUrl}/story`}>
          <span>
            <Icon>
        ...
        <a href={`${customerWebsiteUrl}/story`}>
          <span>
            <Icon>
              <circle cx="12" cy="12" r="9" />
...
function BottomNav({
  ...
  const items: { id: AppTab; label: string; icon: ReactNode }[] = [
    {id:"home",label:"Home",icon:<House size={23} aria-hidden />},
    {id:"menu",label:"Menu",icon:<Coffee size={23} aria-hidden />},
    {id:"orders",label:"Orders",icon:<Receipt size={23} aria-hidden />},
    {id:"rewards",label:"Rewards",icon:<Gift size={23} aria-hidden />},
  ];
  return (
...
function AuthSheet({
  ...
  signup: (displayName: string) => Promise<SignupResult>;
  ...
  async function submit(event: FormEvent) {
    ...
    setBusy(true);
    if (mode === "signup") {
      const result = await signup(displayName);
