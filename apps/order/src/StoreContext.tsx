...
const storageKey = 'coffee-demo-selected-store'
...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  async function reloadStores() {
    ...
    let { data, error } = await supabase
      .from('stores')
      .select('id,name,address,phone,preparation_minutes,minutes_per_cup,buffer_minutes,accepting_pickup,opening_time,closing_time,image_url,state,city,latitude,longitude')
      .eq('active', true)
      .order('name')
    if (error && /opening_time|closing_time|schema cache/i.test(error.message)) {
      const legacy = await supabase
        ...
        .eq('active', true)
        .order('name')
      data = legacy.data?.map(store => ({ ...store, state:'', city:'', latitude:null, longitude:null, opening_time:'10:00:00', closing_time:'22:00:00' })) ?? null
      error = legacy.error
    }
