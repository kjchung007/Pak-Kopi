...
const supabase = url && key ? configureDemoUpdates(createClient(url, key),import.meta.env.VITE_LOCAL_DEMO === "true") : null;
...
function Seal({ logo = brand.logo }: { logo?: string }) {
  return logo ? (
    <img className="seal seal-image" src={logo} alt="Shop logo" />
  ) : (
    <span className="seal">{brand.initials}</span>
  );
}
function Login({
  enter,
  ...
  return (
    <main className="login">
      <section className="login-world">
        ...
        <footer>
          <span>Multi-branch demonstration</span>
          <strong>Malaysia · MYR</strong>
        </footer>
      </section>
      <section className="login-form">
        <form onSubmit={submit}>
          ...
          <h2>Admin sign in</h2>
          <p>Use your protected demo administrator account.</p>
          {error && (
...
function Brand() {
  return (
    <span className="brand">
      <Seal />
      <span>
        <strong>{brand.name}</strong><small>{brand.tagline}</small>
...
function Shell({
    ...
    async function load() {
      ...
      if (profileRows) {
        const teamIds = new Set([
          ...(people ?? []).map((person: any) => person.user_id),
          ...(adminAccounts ?? []).map((account: any) => account.user_id),
        ]);
        ...
        setCustomers(profileRows.filter((profile: any) => !teamIds.has(profile.user_id)).map((profile: any) => {
          const stats = customerStats.get(profile.user_id) ?? { orders: 0, spend: 0, lastOrder: "", email: "" };
          const customerEmail = profile.email || stats.email;
          return { userId: profile.user_id, name: profile.display_name || customerEmail.split("@")[0] || "Customer", email: customerEmail, phone: profile.phone ?? "", orders: stats.orders, spend: stats.spend, lastOrder: stats.lastOrder ? new Date(stats.lastOrder).toLocaleString() : "" };
        }));
      }
  ...
  const heads = {
    overview: ["Good afternoon", `Here is how ${brand.name} is moving today.`],
    ...
    website: ["Website Editor", `Build, preview and publish the ${brand.name} website.`],
    settings: ["Shop settings", "Manage the demo brand and branch defaults."],
  }[page];
...
function Orders({
  ...
  const paymentLabel = (method: Order["paymentMethod"]) => brand.demo ? "Simulated payment" :
    method === "touch_n_go"
      ? "Touch ’n Go"
      : method === "fpx"
        ? "FPX online banking"
    ...
    <section className="campaign-workspace">
      <aside className="panel campaign-list">
        ...
        {campaigns.map((item) => (
          ...
          >
            <span>
              {item.image ? <img src={item.image} alt="" /> : <b>{brand.initials}</b>}
            </span>
            <span>
...
function RewardsManagement({
  ...
  function randomCode() {
    ...
    setCode(`${brand.initials}${[...bytes].map((n) => chars[n % chars.length]).join("")}`);
  }
    ...
    <section className="rewards-admin">
      ...
      {section === "codes" && (
        <div className="reward-admin-grid code-grid">
          ...
          <article className="reward-panel">
            <span className="eyebrow">Demo test codes</span>
            <h2>Ready to claim</h2>
            <div className="launch-codes">
              <code>DEMOWELCOME</code>
              
              <code>DEMOBUY2</code>
            </div>
            <p>
              These illustrative demo codes use the
