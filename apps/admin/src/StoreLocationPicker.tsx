...
export function StoreLocationPicker({latitude,longitude,mapsUrl,address,onChange}:{latitude?:number|null;longitude?:number|null;mapsUrl?:string;address:string;onChange:(value:{latitude:number|null;longitude:number|null;state?:string;city?:string})=>void}){
 ...
 function useMapsLink(value:string){setMapsLink(value);const match=value.match(/@(-?\d+(?:\.\d+)?),\s*(-?\d+(?:\.\d+)?)/)||value.match(/[?&](?:query|q|ll)=(-?\d+(?:\.\d+)?)[,%20]+(-?\d+(?:\.\d+)?)/i);if(match){const lat=Number(match[1]),lon=Number(match[2]);if(Math.abs(lat)<=90&&Math.abs(lon)<=180){onChange({latitude:Number(lat.toFixed(7)),longitude:Number(lon.toFixed(7))});setError('');return;}}if(value.trim())setError('Paste a full Google Maps link containing coordinates, or use Search below.')}
