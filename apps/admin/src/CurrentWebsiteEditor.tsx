 ...
 return <section className="site-workspace">
  ...
  {!doc?<div className="workspace-empty">{busy?'Loading the current website…':'Could not load the draft.'}<button onClick={()=>void load()}>Reload</button></div>:<div className="workspace-grid">
   <aside>
    ...
    <details key={page+"text"}><summary><EditorIcon kind="text"/>Text</summary>{Object.entries(doc.copy).filter(([key])=>key.startsWith(page+'-') && !['home-10','home-14','home-17','home-20','story-19','story-20','story-21','story-22','story-23','story-24'].includes(key)).map(([key,value])=><label key={key}>{defaultSite.copy[key]?.slice(0,55)||key}<textarea value={value} maxLength={10000} onChange={e=>update({...doc,copy:{...doc.copy,[key]:e.target.value}})}/></label>)}</details>
