...
function AnimatedCounter({value}:{value:number}){
 const [display,setDisplay]=useState(value),[flowing,setFlowing]=useState(false);
 ...
 useEffect(()=>{
  ...
  setFlowing(true);
  const flowTimer=window.setTimeout(()=>setFlowing(false),450);
  ...
  return()=>{cancelAnimationFrame(frameId);clearTimeout(flowTimer);};
 },[value]);
 return <strong className={`locator-total-number ${flowing?'flowing':''}`}>{display}</strong>;
}
...
export function StoresView({initial,editing}:{initial:SiteDocument;editing:boolean;orderUrl:string}){
 ...
 const [state,setState]=useState(''),[query,setQuery]=useState(''),[page,setPage]=useState(1);
 ...
 const stateCount=state?stores.filter(s=>normalizeState(s.state)===state).length:stores.length;
 function select(value:string,toggle=false){setState(curr=>toggle&&curr===value?'':value);setQuery('');setPage(1)}
 ...
 return <main className="stores-locator">
  ...
  <section className="locator-finder" id="find-stores" ref={finder} aria-label="Find a store in Malaysia">
   ...
   <div className="locator-map-wrap"><div className="locator-total"><div><AnimatedCounter value={stateCount}/><img src={brand.logo} alt={brand.name}/></div><p key={state||'all'} className="locator-total-caption">Stores {state?`in ${state}`:'across Malaysia'}</p></div><MalaysiaMap selected={state} onSelect={s=>select(s,true)}/></div>
   <div className="locator-results" ref={resultsRef}><div className="locator-results-heading"><h2>{state||'Across Malaysia'}</h2><span role="status">{filtered.length} {filtered.length===1?'store':'stores'}</span></div>
    ...
    {!shown.length&&<div className="locator-empty"><h3>{query?'No matching stores.':'More coffee stops to come.'}</h3><p>{query?'Try another name or address.':'No branches listed in this state yet.'}</p>{(state||query)&&<button type="button" onClick={()=>select('')}>View all branches</button>}</div>}
