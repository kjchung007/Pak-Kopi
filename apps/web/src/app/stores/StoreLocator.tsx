 ...
 return <main className="stores-locator">
  ...
  <section className="locator-finder" id="find-stores" ref={finder} aria-label="Find a store in Malaysia">
   ...
   <div className="locator-results" ref={resultsRef}><div className="locator-results-heading"><h2>{state||'Across Malaysia'}</h2><span role="status">{filtered.length} {filtered.length===1?'store':'stores'}</span></div>
    <div className="locator-grid">{shown.map(s=><article className="locator-card" key={s.id}><img className="locator-photo" src={s.image||brand.storePlaceholder} alt={`${s.name} branch`} loading="lazy" onError={e=>{e.currentTarget.onerror=null;e.currentTarget.src=brand.storePlaceholder}}/><div className="locator-card-body"><h3>{s.name}</h3><p>{s.address}</p><span className="locator-hours"><Clock size={18}/>{hours(s)}</span><a href={'https://www.google.com/maps/search/?api=1&query='+encodeURIComponent(s.latitude!=null&&s.longitude!=null?`${s.latitude},${s.longitude}`:`${brand.name} ${s.name} ${s.address}`)} target="_blank" rel="noreferrer" aria-label={`Directions to ${s.name}`}>Directions<ArrowUpRight size={20}/></a></div></article>)}</div>
    {!shown.length&&<div className="locator-empty"><h3>{query?'No matching stores.':'More coffee stops to come.'}</h3><p>{query?'Try another name or address.':'No branches listed in this state yet.'}</p><button onClick={()=>select(query?state:'Sabah')}>{query?'Clear search':'Browse Sabah'}</button></div>}
